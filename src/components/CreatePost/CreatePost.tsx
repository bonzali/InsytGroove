...
function CreatePost({onClose}: Props) {
    ...
    return (
        <div className={'CreatePost'}>
            <span className={'close-button'} onClick={() => {onClose()}} role={'button'}> <FiX/> </span>
            <h5>Create New Post </h5>
            <input value={values.title} onChange={handleChange('title')} placeholder={'title'}/>
            <textarea value={values.description} rows={4} onChange={handleChange('body')} placeholder={'description'}/>
